...
class CAPM:
    ...
    def download_data(self):
        data = {}
        # for every stock download the ticker data and put it in the data dictionary
        for stock in self.stocks:
            ticker = yf.download(stock, self.start_date, self.end_date, auto_adjust=False)
            data[stock] = ticker['Adj Close']

        # as you return convert the data dictionary into a dataframe
    ...
    def initialize(self):
        ...
        # Ensure the DataFrame has the correct index and 1-dimensional data (last time it returned 2 dimensional form of the column data)
        self.data = pd.DataFrame({
            's_adjclose': stock_data[self.stocks[0]].values.flatten(),
            'm_adjclose': stock_data[self.stocks[1]].values.flatten()
